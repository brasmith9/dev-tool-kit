    ...
    <div className="animate-in flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-auto lg:h-[calc(100vh-190px)]">
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
                    ...
                    {(["v4", "v1", "v5"] as const).map((v) => (
                      <button
                        ...
                        className={`flex-1 py-1.5 rounded-md text-xs font-bold transition-smooth cursor-pointer ${
                          config.version === v ? "bg-surface text-primary shadow-sm" : "text-text-dim hover:text-text-sub"
                        }`}
                      >
                        {v.toUpperCase()}
                      </button>
                    ...
                    {["uppercase", "lowercase"].map((s) => (
                      <label key={s} className="flex items-center gap-2.5 cursor-pointer group">
                        <div className={`size-4 rounded-full border flex items-center justify-center transition-smooth ${
                          ...
                          {config.caseStyle === s && <div className="size-1.5 rounded-full bg-surface" />}
                        </div>
                        <input 
                          type="radio" 
                          className="hidden" 
              ...
              <div className="space-y-4">
                <div className="space-y-3 pt-1">
                  <label className="flex items-center justify-between cursor-pointer group">
                    ...
                    >
                      <div className={`absolute top-1 size-3 rounded-full bg-surface transition-smooth ${config.includeBraces ? "left-5" : "left-1"}`} />
                    </button>
                  </label>
                  <label className="flex items-center justify-between cursor-pointer group">
                    ...
                    >
                      <div className={`absolute top-1 size-3 rounded-full bg-surface transition-smooth ${config.includeHyphens ? "left-5" : "left-1"}`} />
                    </button>
                  </label>
                </div>
          ...
          {/* Output Card */}
          <div className="bg-surface rounded-xl border border-border shadow-card flex-1 flex flex-col min-h-[400px]">
                  ...
                  {guids.map((guid, i) => (
                    <div key={i} className="group flex items-center justify-between p-3.5 bg-surface-raised rounded-xl border border-transparent hover:border-primary/20 transition-smooth">
                      ...
                      <button 
                        ...
                        className="size-8 rounded-lg flex items-center justify-center text-text-dim hover:text-primary hover:bg-surface transition-smooth shadow-sm opacity-0 group-hover:opacity-100 cursor-pointer"
                      >
        ...
        <div className="lg:col-span-4 flex flex-col gap-6">
          <div className="bg-surface rounded-xl border border-border shadow-card flex flex-col h-[calc(100vh-180px)]">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              {history.length === 0 ? (
                ...
                history.map((item) => (
                  <div key={item.id} className="p-3 bg-surface border border-border rounded-xl hover:shadow-sm transition-smooth group relative overflow-hidden">
                    ...
                    <button 
                      ...
                      className="absolute top-2 right-2 size-6 rounded bg-surface shadow-sm border border-border flex items-center justify-center opacity-0 group-hover:opacity-100 transition-smooth hover:text-primary cursor-pointer"
                    >
